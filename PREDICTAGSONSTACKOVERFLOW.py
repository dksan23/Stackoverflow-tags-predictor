...
def readata(filename):
    ...
    return data
train=readata('data/train.tsv')
validation=readata('data/validation.tsv')
test=pd.read_csv('data/test.tsv',sep='\t')
xtrain,ytrain=train['title'].values,train['tags'].values
xvalid,yvalid=validation['title'].values,validation['tags'].values
...
xvalid=[text_prepare(text) for text in xvalid]
...
yvalid=mlv.fit_transform(yvalid)   
...
prediction=classified.predict(xvalid) 
predictionscore=classified.decision_function(xvalid)   
prediction=mlv.inverse_transform(prediction)
yval=mlv.inverse_transform(yval)
yval[3]
prediction[3]
...
print(f1_score(yval, prediction, average='weighted'))
print(average_precision_score(yval, prediction))  
